...
public class LongKeyLinkedMap<V> {
	...
	private synchronized V _intern(long key, int m) {
		LongKeyLinkedEntry<V> tab[] = table;
		int index = hash(key) % tab.length;
		for (LongKeyLinkedEntry<V> e = tab[index]; e != null; e = e.next) {
			if (CompareUtil.equals(e.key, key)) {
	...
	private synchronized V _put(long key, V value, int m) {
		LongKeyLinkedEntry tab[] = table;
		int index = hash(key) % tab.length;
		for (LongKeyLinkedEntry<V> e = tab[index]; e != null; e = e.next) {
			if (CompareUtil.equals(e.key, key)) {
	...
	public synchronized void sort(final boolean asc) {
		this.sort(new Comparator<LongKeyLinkedEntry<V>>() {
			public int compare(LongKeyLinkedEntry<V> o1, LongKeyLinkedEntry<V> o2) {
				if (asc)
	...
	public synchronized void sort(Comparator<LongKeyLinkedEntry<V>> c) {
		ArrayList<LongKeyLinkedEntry<V>> list = new ArrayList<LongKeyLinkedEntry<V>>(this.size());
		Enumeration<LongKeyLinkedEntry<V>> en = this.entries();
		while (en.hasMoreElements()) {
			LongKeyLinkedEntry<V> n=en.nextElement();
			if(n!=null){
				list.add(n);
			}
		}
		...
		for (int i = 0; i < list.size(); i++) {
			LongKeyLinkedEntry<V> e = list.get(i);
			this.put(e.getKey(), e.getValue());
		}
	}
	public  LongKeyLinkedMap<V> sortOut(Comparator<LongKeyLinkedEntry<V>> c) {
		Object[] o;
		synchronized (this) {
		...
		LongKeyLinkedMap<V> out = new LongKeyLinkedMap<V>();
		for (int i = 0; i < o.length; i++) {
			LongKeyLinkedEntry<V> e = (LongKeyLinkedEntry<V>) o[i];
