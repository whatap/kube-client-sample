...
public class ThreadUtil {
	...
	public static String getName(Class clazz) {
		...
		if (name.startsWith("whatap.agent.") == false)
			return name;
		...
		for (int i = 0; i < thread.length; i++) {
			try {
				...
				if (fo != null) {
					...
					long _cpu = tmb.getThreadCpuTime(thread[i]) / 1000000;
	...
	public static ThreadPoolExecutor createExecutor(final String name, int count, int keepAlive, final boolean isDaemon) {
		ThreadPoolExecutor exe = new ThreadPoolExecutor(count, count, keepAlive, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(),
				new ThreadFactory() {
