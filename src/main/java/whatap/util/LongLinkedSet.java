...
public class LongLinkedSet {
	...
	private synchronized long _put(long key, int m) {
		LongLinkedSetry buk[] = table;
		int index = hash(key) % buk.length;
		for (LongLinkedSetry e = buk[index]; e != null; e = e.next) {
			if (CompareUtil.equals(e.key, key)) {
	...
	public void putAll(LongLinkedSet o) {
		LongEnumer e = o.elements();
		while (e.hasMoreElements()) {
			this.put(e.nextLong());
		}
	}
	public void putAll(LongSet o) {
		LongEnumer e = o.values();
		while (e.hasMoreElements()) {
			this.put(e.nextLong());
		}
	}

	public void putAll(long[] arr) {
