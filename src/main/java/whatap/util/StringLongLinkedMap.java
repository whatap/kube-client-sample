...
public class StringLongLinkedMap {
	...
	private synchronized long _put(String key, long value, int m) {
		if (key == null)
			return NONE;
	...
	public long addLast(String key, long value) {
		return _add(key, value, MODE.FORCE_LAST);
	}

	public long addFirst(String key, long value) {
		return _add(key, value, MODE.FORCE_FIRST);
	}
	private synchronized long _add(String key, long value, int m) {
		if (key == null)
			return NONE;
		StringLongLinkedEntry tab[] = table;
		int index = hash(key) % tab.length;
	...
	public synchronized void sort(Comparator<StringLongLinkedEntry> c) {
		ArrayList<StringLongLinkedEntry> list = new ArrayList<StringLongLinkedEntry>(this.size());
		Enumeration<StringLongLinkedEntry> en = this.entries();
		...
		for (int i = 0; i < list.size(); i++) {
			StringLongLinkedEntry e = list.get(i);
