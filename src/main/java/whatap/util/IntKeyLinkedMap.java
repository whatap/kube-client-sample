...
public class IntKeyLinkedMap<V> {
	...
	public IntLinkedSet getKeySet() {
		IntLinkedSet _keys = new IntLinkedSet();
		IntEnumer en = this.keys();
	...
	public synchronized V getLRU(int key) {
		IntKeyLinkedEntry tab[] = table;
		int index = hash(key) % tab.length;
		for (IntKeyLinkedEntry<V> e = tab[index]; e != null; e = e.next) {
			if (CompareUtil.equals(e.key, key)) {
	...
	private synchronized V _intern(int key, int m) {
		IntKeyLinkedEntry tab[] = table;
		int index = hash(key) % tab.length;
		for (IntKeyLinkedEntry<V> e = tab[index]; e != null; e = e.next) {
			if (CompareUtil.equals(e.key, key)) {
	...
	private synchronized V _put(int key, V value, int m) {
		IntKeyLinkedEntry tab[] = table;
		int index = hash(key) % tab.length;
		for (IntKeyLinkedEntry<V> e = tab[index]; e != null; e = e.next) {
			if (CompareUtil.equals(e.key, key)) {
	...
	private class Enumer<V> implements Enumeration, IntEnumer, Iterator<V> {
		byte type;
	...
	public void putAll(IntKeyLinkedMap<V> o) {
		Enumeration<IntKeyLinkedEntry<V>> en = o.entries();
		while (en.hasMoreElements()) {
			IntKeyLinkedEntry<V> n = en.nextElement();
