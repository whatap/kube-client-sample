...
public class CacheTable<K, V> {
	...
	public CacheTable(int initCapacity, float loadFactor) {
		...
		this.header = new CacheEntry(null, null, 0, null);
	...
	public synchronized Enumeration<CacheEntry<K, V>> entries() {
		return new Enumer<CacheEntry>(TYPE.ENTRIES);
	}
	...
	public void clearExpiredItemsWithCheckInterval(long checkInterval) {
		...
		if(now - last_check > checkInterval) {
			last_check = now;		
			clearExpiredItems();		
		}
	...
	private synchronized V _put(K key, V value, long keepTime, int m) {
		...
		CacheEntry e = new CacheEntry(key, value, keepTime, tab[index]);
		tab[index] = e;
		switch (m) {
		case MODE.FORCE_FIRST:
		case MODE.FIRST:
	...
	public String toString() {
		...
		for (int i = 0; it.hasMoreElements(); i++) {
			CacheEntry e = (CacheEntry) (it.nextElement());
			if (i > 0)
				buf.append(", ");
	...
	public static class CacheEntry<K, V> {
		...
		protected Object clone() {
			return new CacheEntry(key, value, keepTime, (next == null ? null : (CacheEntry) next.clone()));
		}
		public K getKey() {
			return key;
		}
	...
	private class Enumer<V> implements Enumeration {
		byte type;
		CacheEntry entry = CacheTable.this.header.link_next;
		CacheEntry lastEnt;
		Enumer(byte type) {
			this.type = type;
		}
