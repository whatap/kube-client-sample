...
public class LongFloatLinkedMap {
	...
	private synchronized float _put(long key, float value, int m) {
		LongFloatLinkedEntry tab[] = table;
		int index = hash(key) % tab.length;
		for (LongFloatLinkedEntry e = tab[index]; e != null; e = e.hash_next) {
			if (CompareUtil.equals(e.key, key)) {
	...
	public float add(long key, float value) {
		return _add(key, value, MODE.LAST);
	}

	public float addLast(long key, float value) {
		return _add(key, value, MODE.FORCE_LAST);
	}

	public float addFirst(long key, float value) {
		return _add(key, value, MODE.FORCE_FIRST);
	}
	private synchronized float _add(long key, float value, int m) {
		LongFloatLinkedEntry tab[] = table;
		int index = hash(key) % tab.length;
		for (LongFloatLinkedEntry e = tab[index]; e != null; e = e.hash_next) {
			if (CompareUtil.equals(e.key, key)) {
	...
	public static class LongFloatLinkedEntry {
		...
		protected Object clone() {
			return new LongFloatLinkedEntry(key, value, (hash_next == null ? null : (LongFloatLinkedEntry) hash_next.clone()));
		}
	...
	public void toBytes(DataOutputX dout) {
		...
		Enumeration<LongFloatLinkedEntry> en = this.entries();
		while (en.hasMoreElements()) {
			LongFloatLinkedEntry e = en.nextElement();
			dout.writeDecimal(e.getKey());
			dout.writeFloat(e.getValue());
		}
	}

	public LongFloatLinkedMap toObject(DataInputX in) {
	...
	public synchronized void sort(Comparator<LongFloatLinkedEntry> c) {
		ArrayList<LongFloatLinkedEntry> list = new ArrayList<LongFloatLinkedEntry>(this.size());
		Enumeration<LongFloatLinkedEntry> en = this.entries();
		while (en.hasMoreElements()) {
		...
		for (int i = 0; i < list.size(); i++) {
			LongFloatLinkedEntry e = list.get(i);
