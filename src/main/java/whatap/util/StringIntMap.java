...
public class StringIntMap {
	...
	public synchronized int intern(String key, int def) {
		if (key == null)
			return NONE;
		StringIntEntry tab[] = table;
		int index = (key.hashCode() & Integer.MAX_VALUE) % tab.length;
		...
		StringIntEntry e = new StringIntEntry(key, def, tab[index]);
		tab[index] = e;
		count++;
		return def;
	}
	...
	public synchronized void sort(Comparator<StringIntEntry> c) {
		ArrayList<StringIntEntry> list = new ArrayList<StringIntEntry>(this.size());
		Enumeration<StringIntEntry> en = this.entries();
