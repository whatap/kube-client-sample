...
public class DataOutputX {
	...
	public final static long LONG6_MIN_VALUE = 0xffff800000000000L;
	public final static long LONG6_MAX_VALUE = 0x00007fffffffffffL;
	private int written;
	...
	// public DataOutputX flushShortBytes(byte[] b){
	...
	public DataOutputX writeBlob(byte[] value) {
		if (value == null || value.length == 0) {
			...
			if (len <= 253) {
				...
				byte[] buff = new byte[3];
				buff[0] = (byte) 255; // 255 means value's length is more than
										// 253 bytes.
				write(toBytes(buff, 1, (short) len));
				write(value);
			} else {
				byte[] buff = new byte[5];
				buff[0] = (byte) 254; // 254 means value's length is more than
										// 65535 bytes.
				write(toBytes(buff, 1, len));
				write(value);
			}
	...
	private final static byte[] obfusKey = new byte[] { (byte) 0xff, 0x10, (byte) 0xf2, 0x77, 0x49 };
	static byte[] obfus(byte[] b) {
		for (int i = 0, j = 0; i < b.length; i++, j = (j + 1) % obfusKey.length) {
			b[i] ^= obfusKey[j];
		}
		return b;
	}
